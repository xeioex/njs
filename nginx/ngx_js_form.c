...
static ngx_uint_t ngx_js_form_is_hex(u_char ch);
    ...
    ngx_str_t *error)
{
    ...
    case NGX_JS_FORM_URLENCODED:
        rc = ngx_js_form_parse_urlencoded(pool, body, len, max_keys, f,
                                          error);
        break;
    ...
    ngx_uint_t max_keys, ngx_js_form_t *form, ngx_str_t *error)
{
    ...
    while (p < end) {
        ...
        if (ngx_js_form_add_entry(form, pool, &name, &value, &count,
                                  max_keys, 0, error)
            != NGX_JS_FORM_OK)
        {
            return NGX_JS_FORM_PARSE_ERROR;
...
static ngx_int_t
ngx_js_form_add_entry(ngx_js_form_t *form, ngx_pool_t *pool,
    ngx_str_t *name, ngx_str_t *value, ngx_uint_t *count, ngx_uint_t max_keys,
    ...
    ngx_str_t *dst, ngx_str_t *error)
{
    ...
    for (p = start; p < end; p++) {
        ...
        if (*p == '%') {
            if (p + 2 >= end || !ngx_js_form_is_hex(p[1])
                || !ngx_js_form_is_hex(p[2]))
            {
                ngx_js_form_error(error, "malformed percent escape");
                return NGX_JS_FORM_PARSE_ERROR;
            }

            *d++ = (u_char) ((ngx_hextoi(p + 1, 2)) & 0xff);
            p += 2;
            continue;
        }
