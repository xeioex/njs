    ...
    ngx_uint_t *max_keys)
{
    ...
    *max_keys = NGX_JS_FORM_DEFAULT_MAX_KEYS;
    ...
    if (num < 1 || num > (double) NGX_MAX_UINT32_VALUE) {
        njs_vm_type_error(vm, "\"maxKeys\" must be a positive integer");
        return NJS_ERROR;
    }

    *max_keys = (ngx_uint_t) num;
    ...
    ngx_uint_t *max_keys)
{
    ...
    *max_keys = NGX_JS_FORM_DEFAULT_MAX_KEYS;
    ...
    if (JS_ToFloat64(cx, &num, value) < 0) {
        JS_FreeValue(cx, value);
    ...
    if ((double) (ngx_uint_t) num != num
        || num < 1 || num > (double) NGX_MAX_UINT32_VALUE)
    {
        JS_ThrowTypeError(cx, "\"maxKeys\" must be a positive integer");
        return NGX_ERROR;
    }
