...
static int ngx_http_qjs_headers_out_own_property(JSContext *cx,
    JSPropertyDescriptor *pdesc, JSValueConst obj, JSAtom prop);
static int ngx_http_qjs_headers_out_own_property_names(JSContext *ctx,
    JSPropertyEnum **ptab, uint32_t *plen, JSValueConst obj);
...
ngx_http_js_init_vm(ngx_http_request_t *r, njs_int_t proto_id)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ctx->log, 0,
                   "http js vm clone: %p from: %p", ctx->engine, jlcf->engine);
...
ngx_http_qjs_ext_args(JSContext *cx, JSValueConst this_val)
{
    ...
    args = ngx_qjs_arg(ctx->rargs);

    if (!JS_IsUndefined(ngx_qjs_arg(args))) {
        return JS_DupValue(cx, ngx_qjs_arg(ctx->rargs));
    }
...
static JSValue
ngx_http_qjs_ext_log(JSContext *cx, JSValueConst this_val,
    int argc, JSValueConst *argv, int level)
{
    int                  n;
    const char          *msg;
...
ngx_http_qjs_ext_response_body(JSContext *cx, JSValueConst this_val, int type)
{
    ...
    if (!JS_IsUndefined(ngx_qjs_arg(ctx->response_body))) {
        if ((buffer_type == NGX_JS_STRING)
            == JS_IsString(ngx_qjs_arg(ctx->response_body)))
        {
            return JS_DupValue(cx, ngx_qjs_arg(ctx->response_body));
        }
    }
    ...
    ngx_qjs_arg(ctx->response_body) = body;
...
ngx_http_qjs_ext_request_body(JSContext *cx, JSValueConst this_val, int type)
{
    ...
    if (!JS_IsUndefined(ngx_qjs_arg(ctx->request_body))) {
        if ((buffer_type == NGX_JS_STRING)
            == JS_IsString(ngx_qjs_arg(ctx->request_body)))
        {
            return JS_DupValue(cx, ngx_qjs_arg(ctx->request_body));
        }
    }
    ...
    ngx_qjs_arg(ctx->request_body) = body;
...
ngx_http_qjs_ext_variables(JSContext *cx, JSValueConst this_val, int type)
{
    ...
    /*
     * ngx_http_request_t is always aligned to 16 bytes, so we can use the
     * lower 4 bits of the pointer to store the buffer type.
     */
    ...
    JSValueConst obj, JSAtom prop)
{
    ...
    buffer_type = ngx_js_buffer_type((uintptr_t) r & 15);
    r = (ngx_http_request_t *) ((uintptr_t) r & ~15Ul);
...
static int
ngx_http_qjs_headers_out_set_property(JSContext *cx, JSValueConst obj,
    JSAtom prop, JSValueConst value, JSValueConst receiver, int flags)
{
    int                   ret;
    ngx_str_t             name;
    ...
    njs_int_t proto_id, void *external)
{
    ...
    if (!JS_IsRegisteredClass(JS_GetRuntime(cx),
                              ngx_http_qjs_variables_class_id))
    {
    ...
    ngx_qjs_arg(hctx->retval) = JS_UNDEFINED;
    ngx_qjs_arg(hctx->rargs) = JS_UNDEFINED;
    ngx_qjs_arg(hctx->request_body) = JS_UNDEFINED;
    ngx_qjs_arg(hctx->response_body) = JS_UNDEFINED;

    JS_SetOpaque(ngx_qjs_arg(hctx->args[0]), external);
    ...
    ngx_js_loc_conf_t *conf)
{
    ...
    if (ctx != NULL) {
        ...
        JS_FreeValue(e->u.qjs.ctx, ngx_qjs_arg(hctx->rargs));
        JS_FreeValue(e->u.qjs.ctx, ngx_qjs_arg(hctx->request_body));
        JS_FreeValue(e->u.qjs.ctx, ngx_qjs_arg(hctx->response_body));
    }

    ngx_qjs_destroy(e, ctx, conf);
}
...
ngx_http_js_init_conf_vm(ngx_conf_t *cf, ngx_js_loc_conf_t *conf)
{
    ...
    if (conf->type == NGX_ENGINE_NJS) {
        ...
        options.destroy = ngx_engine_qjs_destroy;
    }
#endif
