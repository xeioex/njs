...
static njs_int_t njs_parser_optional_chain_after(njs_parser_t *parser,
    njs_lexer_token_t *token, njs_queue_link_t *current);
static njs_int_t njs_parser_optional_chain_wrap(njs_parser_t *parser,
    njs_lexer_token_t *token, njs_queue_link_t *current);
    ...
    uint8_t ctor)
{
    ...
    case NJS_TOKEN_OPTIONAL_CHAIN:
            ...
            && node->right->token_type == NJS_TOKEN_PROPERTY)
        {
            ...
            prop->u.operation = node->right->u.operation;
            prop->token_line = node->right->token_line;
            prop->left = node->left;
            prop->right = node->right->right;
    ...
    njs_lexer_token_t *token, njs_queue_link_t *current)
{
    ...
    opt->u.operation = NJS_VMCODE_OPTIONAL_CHAIN;
    opt->left = parser->node;
    opt->left->dest = opt;

    ref = njs_parser_node_new(parser, NJS_TOKEN_OPTIONAL_CHAIN_REF);
    if (ref == NULL) {
        return NJS_ERROR;
    }

    ref->token_line = token->line;
    ref->left = parser->node;
    parser->node = ref;
        ...
        if (node != NULL
            && (node->token_type == NJS_TOKEN_PROPERTY
                || (node->token_type == NJS_TOKEN_OPTIONAL_CHAIN
                    ...
                    && node->right->token_type == NJS_TOKEN_PROPERTY)))
        {
            /* o.m?.() or o?.m?.() METHOD_CALL for this. */

            node = njs_parser_node_new(parser, NJS_TOKEN_OPTIONAL_CHAIN_REF);
            if (node == NULL) {
                return NJS_ERROR;
            }
            ...
            prop_node = njs_parser_node_new(parser,
                                            NJS_TOKEN_OPTIONAL_CHAIN_REF);
            if (prop_node == NULL) {
                return NJS_ERROR;
            }
            ...
            func = njs_parser_node_new(parser, NJS_TOKEN_PROPERTY);
            if (func == NULL) {
                return NJS_ERROR;
            }

            func->left = node;
            func->right = prop_node;
            func->token_line = token->line;
