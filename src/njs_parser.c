...
njs_parser(njs_vm_t *vm, njs_parser_t *parser)
{
    ...
    njs_string_get(&njs_predefined.vs._undefined, &str);
    ...
    uint32_t line, const char *fmt, va_list args)
{
    ...
    njs_value_property_set(vm, &error, njs_value_arg(&njs_predefined.vs._lineNumber), &value);

    if (file->length != 0) {
        ret = njs_string_create(vm, &value, file->start, file->length);
        if (ret == NJS_OK) {
            njs_value_property_set(vm, &error, njs_value_arg(&njs_predefined.vs._fileName),
                                   &value);
        }
    }
