...
njs_object_to_json_function(njs_vm_t *vm, njs_value_t *value)
{
    ...
    if (njs_is_object(value)) {
        njs_object_property_init(&lhq, &njs_predefined.vs._toJSON,
                                 NJS_TO_JSON_HASH);
...
njs_json_stringify_array(njs_json_stringify_t *stringify)
{
    ...
    njs_value_assign(item, &njs_predefined.vs._);
    ...
    const njs_value_t *value)
{
    ...
    prop = njs_object_prop_alloc(vm, &njs_predefined.vs._, value, 1);
    if (njs_slow_path(prop == NULL)) {
        return NULL;
    }
    ...
    njs_value_t *value, njs_uint_t console)
{
    ...
    case NJS_FUNCTION:
        ...
        ret = njs_value_property(stringify->vm, value,
                                 njs_value_arg(&njs_predefined.vs._name), &tag);
        if (njs_slow_path(ret == NJS_ERROR)) {
            return ret;
        }
        ...
        if (njs_is_accessor_descriptor(prop)) {
            if (njs_prop_getter(prop) != NULL) {
                if (njs_prop_setter(prop) != NULL) {
                    val = njs_value_arg(&njs_predefined.vs.__Getter_Setter_);

                } else {
                    val = njs_value_arg(&njs_predefined.vs.__Getter_);
                }

            } else {
                val = njs_value_arg(&njs_predefined.vs.__Setter_);
            }
        }
