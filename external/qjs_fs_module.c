...
static int qjs_fs_stat_get_own_property(JSContext *cx,
    JSPropertyDescriptor *pdesc, JSValueConst obj, JSAtom prop);
static int qjs_fs_stat_get_own_property_names(JSContext *cx,
    JSPropertyEnum **ptab, uint32_t *plen, JSValueConst obj);
static void qjs_fs_stat_finalizer(JSRuntime *rt, JSValue val);
...
static const JSCFunctionListEntry qjs_fs_stat_proto[] = {
...
static JSClassDef qjs_fs_stat_class = {
    "Stat",
    .finalizer = qjs_fs_stat_finalizer,
    .exotic = & (JSClassExoticMethods) {
        .get_own_property = qjs_fs_stat_get_own_property,
        .get_own_property_names = qjs_fs_stat_get_own_property_names,
    },
};
    ...
    JSValueConst *argv, int calltype)
{
    ...
    return qjs_fs_result(cx, result, calltype, callback, 1);
}
    ...
    JSValueConst *argv, int calltype)
{
    ...
    if (encoding == NULL) {
        result = qjs_buffer_create(cx, str.start, str.length);
        ...
        result = JS_NewStringLen(cx, (const char *) data.start, data.length);
        js_free(cx, str.start);
        js_free(cx, data.start);

    } else {
        result = JS_NewStringLen(cx, (const char *) str.start, str.length);
        js_free(cx, str.start);
    }
...
qjs_fs_stats_create(JSContext *cx, struct stat *st)
{
    ...
    obj = JS_NewObjectClass(cx, QJS_CORE_CLASS_ID_FS_STAT);
    ...
    int magic)
{
    ...
    if ((magic >> 2) != QJS_FS_FSTAT) {
        ...
        fd_offset = !!(calltype == QJS_FS_DIRECT);
    ...
    if (ret != 0) {
        ...
        return qjs_fs_result(cx, result, calltype, callback, 2);
    }
    ...
    JSValueConst *argv, int testtype)
{
    ...
    st = JS_GetOpaque2(cx, this_val, QJS_CORE_CLASS_ID_FS_STAT);
    if (st == NULL) {
        return JS_EXCEPTION;
    }
    ...
    const char *path, int errn)
{
    ...
    if (errn != 0) {
        if (JS_SetPropertyStr(cx, value, "errno",
                              JS_NewInt32(cx, errn)) < 0)
        {
            JS_FreeValue(cx, value);
            return JS_EXCEPTION;
        }
...
static int
qjs_fs_stat_get_own_property(JSContext *cx, JSPropertyDescriptor *pdesc,
...
#define qjs_fs_time_ms(ts) ((ts)->tv_sec * 1000.0 + (ts)->tv_nsec / 1000000.0)
    ...
    st = JS_GetOpaque2(cx, obj, QJS_CORE_CLASS_ID_FS_STAT);
    if (st == NULL) {
...
static int
qjs_fs_stat_get_own_property_names(JSContext *cx, JSPropertyEnum **ptab,
...
static void
qjs_fs_stat_finalizer(JSRuntime *rt, JSValue val)
{
    qjs_stat_t  *stat;

    stat = JS_GetOpaque(val, QJS_CORE_CLASS_ID_FS_STAT);
    if (stat != NULL) {
        js_free_rt(rt, stat);
    }
}
...
qjs_fs_init(JSContext *cx, const char *name)
{
    ...
    if (!JS_IsRegisteredClass(JS_GetRuntime(cx),
                              QJS_CORE_CLASS_ID_FS_STAT))
    {
        if (JS_NewClass(JS_GetRuntime(cx), QJS_CORE_CLASS_ID_FS_STAT,
                        &qjs_fs_stat_class) < 0)
        {
            return NULL;
        }
        ...
        JS_SetPropertyFunctionList(cx, proto, qjs_fs_stat_proto,
                                   njs_nitems(qjs_fs_stat_proto));

        JS_SetClassProto(cx, QJS_CORE_CLASS_ID_FS_STAT, proto);
    }
